...
def getDataSet():
    ...
    trainDataSet = np.array(train_data)
    pruneDataSet = np.array(test_data)
    features = labels
    return trainDataSet, pruneDataSet, features
...
if __name__ == '__main__':
    trainData, pruneData, labelList = getDataSet()
    # 用训练集训练一颗树并画图
    ...
    # 计算未剪枝的精度
    print(f"test accuracy = {calAccuracy(pruneData, labelList, myTree)}\n")
    # 计算预剪枝精度
    print(f"test accuracy = {calAccuracy(pruneData, labelList, preTree)}\n")
    # 计算后剪枝精度
    print(f"test accuracy = {calAccuracy(pruneData, labelList, postPTree)}\n")
